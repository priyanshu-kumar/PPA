...
def apply_for_project(request, proj_id):
    ...
    if(proj.deadline_passed()):
        ...
        appl = Application(student=stu,
            ...
            message_from_prof='',
            message_to_prof='',
            accept_datetime = datetime.datetime.max)
        appl.save()
...
def accept_project(request, app_id):
    ...
    appl.accept_datetime = datetime.datetime.max
    appl.save()
        ...
        for key, value in request.POST.items():
            ...
            if(request.POST.get('update_btn')!=None):
                if((key).startswith('status')):
                    ...
                    appl.message_from_prof = request.POST.get('message_from_prof')
...
def save_new_project(request):
    ...
    if(request.method == "POST"):
        ...
        date3 = datetime.datetime.strptime(date2, "%m/%d/%Y")
